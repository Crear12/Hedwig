            ...
            if (nav?.share) {
                ...
                try {
                    ...
                    if (nav.canShare?.({ files: [f] })) {
                        await nav.share({ title: `Hedwig: ${title}`, files: [f] });
                        return;
                    }
                } catch {
                    // ignore and fall back to text share
                }
                await nav.share({ title: `Hedwig: ${title}`, text: md });
                return;
            }
        } catch {
            // fall back to copy
    ...
    async function shareThreadJson() {
        ...
        try {
            ...
            if (nav?.share) {
                ...
                await nav.share({ title: `Hedwig: ${title}`, text: json });
                return;
            }
        } catch {
    ...
    async function shareThreadHtml() {
        ...
        try {
            ...
            if (nav?.share) {
                ...
                await nav.share({ title: `Hedwig: ${title}`, text: html });
                return;
            }
        } catch {
            // fall back to download
